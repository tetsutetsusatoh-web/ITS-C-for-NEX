...
# ============================================================
# [Task4] 曜日別・平休日別baseline対応
# jpholiday が未インストールの場合は pip install jpholiday
# ============================================================
try:
    import jpholiday
...
def get_daytype(date_str):
    """
    日付文字列 (YYYY-MM-DD) から曜日区分を返す。
    返り値: 'weekday' / 'saturday' / 'holiday'
    祝日判定は jpholiday を使用。未インストール時は曜日のみで判定。
    """
    dt = pd.Timestamp(date_str)
    dow = dt.dayofweek  # 0=月, 5=土, 6=日
    if _JPHOLIDAY_AVAILABLE and jpholiday.is_holiday(dt.date()):
        return 'holiday'
    if dow == 6:  # 日曜
        return 'holiday'
    if dow == 5:  # 土曜
...
def build_baseline_and_classify(all_agg, all_waves, output_dir, cfg, logger, save_outputs=True):
    ...
    # ============================================================
    # [Task4] daytype カラムを付与（平日/土曜/休日）
    # baseline を daytype 別に分離することで曜日・平休日差を吸収する
    # ============================================================
    all_agg['daytype'] = all_agg['date'].apply(get_daytype)
    logger.info(f"[Task4] daytype distribution:\n{all_agg[['date','daytype']].drop_duplicates()['daytype'].value_counts().to_dict()}")

    baseline = all_agg.groupby(['direction', 'daytype', 'tod', 'cell100']).agg(
    ...
    # daytype を考慮してmerge（同一曜日区分のbaseline と比較）
    merged = all_agg.merge(baseline, on=['direction', 'daytype', 'tod', 'cell100'], how='left')
...
def evaluate_with_incidents(waves_all, incident_df, output_dir, cfg, logger, save_outputs=True):
    ...
    if len(matched):
        ...
        # 誤警報率: FP / (FP + TN) の代替として FP / total_waves を使用
        far = fp / len(w) if len(w) > 0 else np.nan
        ...
        evaluation = pd.DataFrame([{
            ...
            'f1_score': f1,                          # [Task6] 追加
            'false_alarm_rate': far,                  # [Task6] 追加
        ...
        evaluation = pd.DataFrame([{
            ...
            'f1_score': 0.0,                         # [Task6] 追加
            'false_alarm_rate': (int((w['wave_class'] == 'incident_like').sum()) / len(w)
                                 if len(w) > 0 else np.nan),  # [Task6] 追加
            'mean_kp_diff_km': np.nan,
            'mean_time_gap_min': np.nan
        }])

    # ============================================================
    # [Task6] incident種別別 precision / recall を追加
    # ============================================================
    type_eval = pd.DataFrame()
    if len(matched) and 'incident_name' in matched.columns:
        rows = []
        all_incident_names = inc['incident_name'].dropna().unique() if 'incident_name' in inc.columns else []
        for nm in all_incident_names:
            inc_group = inc[inc['incident_name'] == nm]
            matched_group = matched[matched['incident_name'] == nm]
            matched_incident_like = matched_group[matched_group['wave_class'] == 'incident_like']

            tp_t = len(matched_incident_like)
            fn_t = len(inc_group['incident_id'].dropna().unique()) - len(
                set(matched_group['incident_id'].dropna().tolist())
            )
            fp_t = len(matched_group[matched_group['wave_class'] != 'incident_like'])
            ...
            f1_t = (2 * precision_t * recall_t / (precision_t + recall_t)
                    if pd.notna(precision_t) and pd.notna(recall_t) and (precision_t + recall_t) > 0
                    else np.nan)

            rows.append({
                'incident_name': nm,
                'total_incidents': len(inc_group),
                'matched_rows': len(matched_group),
                'tp': tp_t,
                'fp': fp_t,
                'fn': fn_t,
                'precision': precision_t,             # [Task6] 追加
                'recall': recall_t,                   # [Task6] 追加
                'f1_score': f1_t,                     # [Task6] 追加
                'mean_kp_diff_km': matched_group['kp_diff_km'].mean() if 'kp_diff_km' in matched_group.columns else np.nan,
                'mean_time_gap_min': matched_group['time_gap_min'].mean() if 'time_gap_min' in matched_group.columns else np.nan
            })
...
def run_sensitivity(input_dir, output_dir, base_cfg, grid, logger, incident_df=None):
    ...
    for cfg in variants:
        ...
        compare_rows.append({
            ...
            'f1_score': evaluation.iloc[0]['f1_score'] if len(evaluation) else np.nan,              # [Task6] 追加
            'false_alarm_rate': evaluation.iloc[0]['false_alarm_rate'] if len(evaluation) else np.nan  # [Task6] 追加
        })
